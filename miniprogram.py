...
<style>
    ...
    * {
    ...
    /* 【关键】仅作用于数量控制器内的列，强制3等分水平排列，优先级最高 */
    ...
    @media only screen and (max-width: 768px) {
        ...
        /* 【关键】仅让菜品卡片的外层列在手机端换行，不影响数量控制器 */
        .dish-grid-container > [data-testid="column"] {
...
DISHES = [
    {"id": 101, "category_id": 1, "name": "麻婆豆腐盖浇饭", "price": 28, "img": "https://images.unsplash.com/photo-1588166524941-3bf61a9c41db?w=400&h=300&fit=crop", "desc": "麻辣鲜香，嫩滑入味"},
...
for idx, cat in enumerate(CATEGORIES):
    with top_cat_cols[idx]:
        if st.button(cat["name"], key=f"top_cat_{cat['id']}", use_container_width=True):
...
# 【关键】给菜品外层列加专属容器，限定CSS作用域
st.markdown('<div class="dish-grid-container">', unsafe_allow_html=True)
cols = st.columns(4)
for idx, dish in enumerate(current_dishes):
    with cols[idx % 4]:
        ...
        col_minus, col_num, col_plus = st.columns(3)
        with col_minus:
